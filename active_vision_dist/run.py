...
Tracking args:
  ...
  --port /dev/ttyUSB0 --baud 1000000 (Linux)
...
Keyboard while tracking:
  c = recalibrate   kp=n   kd=n   acc=n   q = quit
"""

import sys, os, time, threading, argparse, csv, json
import numpy as np

# ---- SDK path (current directory, not parent) ----
...
# ============================================================================
# Default config  (port auto-detected: COM6 on Windows, /dev/ttyUSB0 on Linux)
# ============================================================================
H_ID, V_ID = 1, 2
PORT = "COM6" if sys.platform.startswith("win") else "/dev/ttyUSB0"
BAUD, ACC = 1000000, 80
...
# ============================================================================
# PD Controller — Hybrid: large-error jump + small-error PD
# ============================================================================
...
H_FACTOR = int(YAW_RANGE / 360.0 * 4096)     # ~1024 steps
V_FACTOR = int(PITCH_RANGE / 360.0 * 4096)    # ~683 steps
...
def mode_tune_servo(args):
    ...
    for sid, name in [(args.h_id, "Horizontal"), (args.v_id, "Vertical")]:
        _, r, _ = pkt.ping(sid)
        if r != COMM_SUCCESS: print("ERROR: Servo %d no response" % sid); port.closePort(); return
    ...
    for sid, name in [(args.h_id, "Horizontal"), (args.v_id, "Vertical")]:
        ...
        pkt.write1ByteTxRx(sid, 0x37, 0); time.sleep(0.05)
        pkt.write1ByteTxRx(sid, 0x16, 10); time.sleep(0.05)
        pkt.write1ByteTxRx(sid, 0x1A, 8);  time.sleep(0.05)
        pkt.write1ByteTxRx(sid, 0x1B, 8);  time.sleep(0.05)
        pkt.write1ByteTxRx(sid, 0x37, 1);  time.sleep(0.1)
...
def mode_track(args):
    ...
    try:
        with PicoBridge(**pico_kw) as pico:
            ...
            print("[CMDS] kp 1.8 | kd 0.5 | acc 30 | c | q\n")
            ...
            while True:
                while kbhit():
                    ...
                    if key in ("q", "quit", "exit"): raise KeyboardInterrupt
                        ...
                        try:
                            g_kp = float(key.split()[1]); g_kp = max(0.1, min(10., g_kp))
                            pd_ctrl.kp = g_kp; print("[KP] %.1f" % g_kp)
                        except: print("[KP] invalid")
                    elif key.startswith("kd "):
                        try:
                            g_kd = float(key.split()[1]); g_kd = max(0., min(2., g_kd))
                            pd_ctrl.kd = g_kd; print("[KD] %.2f" % g_kd)
                        except: print("[KD] invalid")
                    elif key.startswith("acc "):
                        try:
                            g_acc = int(key.split()[1]); g_acc = max(0, min(254, g_acc))
                            gimbal.acc = g_acc; print("[ACC] %d" % g_acc)
...
def main():
    ...
    p.add_argument("--port", default=saved_port, help="COM port (auto: COM6 win / /dev/ttyUSB0 linux)")
