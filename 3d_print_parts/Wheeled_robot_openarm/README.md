...
本目录整理自 `Wheeled_robot_openarm.zip` 中的同名 STL 备份，用于保存轮式机器人 OpenArm 相关的二自由度主动视觉机构和手部连接件。上传副本保留了原有中文目录、文件名与版本痕迹，未替换或删除仓库中已有的 3D 打印文件。

## 内容概览

| 目录 | 文件数 | 内容说明 |
|---|---:|---|
| `主动视觉/` | 6 | 二自由度头部安装件及早期部件；文件名中的 `3dof` 为历史误命名 |
...
本目录中的主动视觉机构统一为 **二自由度（2-DOF）**：

1. 水平旋转轴：Yaw / Pan；
2. 俯仰旋转轴：Pitch / Tilt。

没有第三个独立旋转轴或移动轴。自由度应按完整机构中相互独立的运动轴判断；单个 STL 只是静态零件，本身不能通过文件名定义机构自由度。

几何复核了全部 9 个名称中含 `004_3dof_head` 的 STL：

| 历史文件名类别 | 数量 | 几何内容 | 正确归属 |
|---|---:|---|---|
| 名称含 `副本` 或 `副本0717` | 4 | 主支架/安装架的不同版本 | 二自由度主动视觉机构部件 |
| 名称含 `旋转支架` | 5 | 旋转支架的版本、孔径和尺寸变体 | 二自由度机构其中一个旋转轴的安装部件 |

因此，`004_3dof_head` 仅是早期不规范的历史文件名，**不表示三自由度**。为保留设计来源和版本追踪，STL 原始文件名暂不改写；文档和后续新增文件应统一使用 `2-DOF`、`2dof` 或 `yaw_pitch` 命名。
...
- 目录名中的日期、`test`、`旧方案更新`、`废弃` 和文件名中的 `3dof` 均来自原始设计记录，其中 `3dof` 已确认为历史误命名。仓库未指定最终量产版本；选择打印件前请先核对实际硬件、孔径和安装方向。
